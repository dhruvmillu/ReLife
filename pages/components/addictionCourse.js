  ...
  useEffect(() =>{
    onAuthStateChanged(auth, (userC) => {
      if (userC) {
        ...
        setUser(userC)
        const load = async () =>{
          const res = await getDoc(doc(db,"info",userC.email,"sub-info","addiction"))
    ...
    <div style={{ display: "flex" }}>
      ...
      <div className={bstyles.rSteps}>
        ...
        <div>
          ...
          <Card
            style={{ border: "1px solid #a0a0a0" ,marginTop:"40px"}}
            sx={{
              minWidth: 255,
              maxWidth: 300,
              boxShadow: "10px 10px 10px #000",
          ...
          >
            <CardContent>
              ...
              <Typography sx={{ mb: 2.5 }} color="text.secondary">
                Meditation improves concentration and relives stress
              </Typography>
              <Typography variant="body2">Sessions attended - {data?.progress.meditation.sessions_attended}</Typography>
            </CardContent>
          </Card>
        </div>
      </div>
...
export default AddictionCourse